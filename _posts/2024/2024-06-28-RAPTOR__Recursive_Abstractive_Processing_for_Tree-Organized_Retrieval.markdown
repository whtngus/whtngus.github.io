...
*문서에서 다양한 추상화 수준에서 정보를 통합할 수 있게 해 주며, 특히 복잡한 다단계 추론이 필요한 질문-응답(QA) 작업에서 최고의 성능을 보인다고함*
...
*기존의 정보 검색 방식은 짧은 텍스트만을 검색하고 활용하는 데 한계가 있음*
...
*RAPTOR는 텍스트 덩어리를 초기에 임베딩하고, 이를 클러스터링하여 요약*

*이 과정은 여러 단계에 걸쳐 반복되며, 최종적으로 다층 트리 구조를 형성*

```
1. 텍스트 세그먼트의 준비
문서를 100 토큰 길이의 짧은 텍스트 덩어리로 나눕니다. 이렇게 나누어진 텍스트는 '리프 노드'로 처리되며, 각 노드는 독립적인 정보 블록으로 간주됩니다.

2. 텍스트 임베딩
각 텍스트 덩어리는 SBERT(Sentence-BERT)를 사용하여 벡터 임베딩됩니다. 이 임베딩은 텍스트의 의미적 유사성을 수치화하여 클러스터링 과정에서 사용됩니다.

3. 클러스터링
유사한 텍스트 덩어리들은 클러스터로 그룹화됩니다. 이 과정은 가우시안 혼합 모델(GMM)을 사용하여 수행되며, 각 클러스터는 텍스트 덩어리들의 유사성에 기반합니다.

4. 요약 생성
클러스터된 텍스트 덩어리들은 언어 모델(GPT-3.5-turbo 사용 예시)을 통해 요약됩니다. 
이 요약은 다시 임베딩되어 상위 노드로 사용됩니다.

5. 재귀적 반복
요약된 텍스트는 다시 임베딩되고 클러스터링되며, 이 과정은 더 이상 클러스터링이 불가능할 때까지 반복됩니다. 
결과적으로, 이 트리는 다양한 추상화 수준을 가진 정보의 계층적 구조를 형성합니다
```
