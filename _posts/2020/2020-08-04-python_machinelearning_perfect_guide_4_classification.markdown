---
layout: post
title: "python_machinelearning_perfect_guide 4_classification"
...
<img src="/img/book/python_machinelearning_perfect_guide/decision_tree_01.PNG" width="400px" height="300px"></img> <br>
>> 출처 wiki <br>
```
...
```
<img src="/img/book/python_machinelearning_perfect_guide/decision_tree_code_result_01.PNG" width="400px" height="300px"></img> <br>
...
```
스태킹 
개별적인 여러 알고리즘을 서로 결합해 예측 결과를 도출함 -> 배깅과 같은점
개별 알고리즘으로 예측한 데이터를 기반으로 다시 예측을 수행 -> 배깅과 다른점
스태킹 모델은 두 종류의 모델이 필요 
1. 개별적인 기반 모델
2. 개별 기반 모델의 예측 데이터를 학습 데이터로 만들어서 학습하는 최종 파라미터

많은 개별 모델이 필요하여 많이 사용하지 않지만 캐글과 같은 정확도를 올리기 위한 대회에서 사용됨

    - CV 세트 기반의 스태킹
과적합을 개선하기 위해 최종 메타 모델을 위한 데이터 세트를 만ㄹ 때 교차 검증 기반으로 예측된 결과 데이터 세트를 이용
- 스텝 1
